...
| `@fsdx/ui-spa/table` | 表格 | `ProTable`（`ProColumnType` / `ProTableProps`，增强 antd Table，`valueType` 支持 `text` / `dateTime` / `dateTimeMinute`，`emptyText` 为空值提供占位内容，可继承页面骨架注入的表体高度）、`TableOperate`（操作列容器：`Edit` / `Delete` / `Link` / `Custom`，按钮统一「图标 + 文字」风格，支持 `disabledReason` 置灰说明）、`SortOrderCell` / `PublishSwitchCell`（单元格内联编辑，乐观更新 + 失败回滚）、`StatusTag`（状态语义色）、`ImageCell`（图片列固定正方形 + contain）、`TableHeightProvider` / `useTableBodyHeight` / `useTableHeight`（表体高度实测与注入） |
