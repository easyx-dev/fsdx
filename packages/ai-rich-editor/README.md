...
AI 驱动的「代码编辑 + 实时预览」三栏工作台（重客户端组件）。对标仓库现有 `RichEditor`（WangEditor 传统富文本）的 AI 进化形态：由对话生成自由 HTML/低代码内容，所见即所得预览。
...
| 依赖 | peer：`antd` / `@ant-design/icons` / `monaco-editor`；dep：`@monaco-editor/react` |
...
export type AiChatAdapter = (
  request: AiChatRequest,   // { messages, mode, snapshot?, options? }
  signal: AbortSignal,
...
- system 提示词由**适配方注入**（可用本包导出的 `DEFAULT_SYSTEM_PROMPT_TEMPLATE` / `MODE_PROMPT_DESCRIPTIONS` 模板）
...
`pnpm --filter @fsdx/ai-rich-editor test`。覆盖代码块提取、预览文档外壳、SSE 帧解析与流消费。

## 相关文档

- 组件方案与演进记录：`.opencode/plan/ai-html-editor-design.md`
