...
AI 驱动的「代码编辑 + 实时预览」工作台（重客户端组件）。定位为**另一种形态的富文本**：只产出可嵌入内容字段的 HTML 片段（fragment），不输出整页文档。对标仓库现有 `RichEditor`（`@easyx/editor` 传统富文本）的 AI 进化形态：由对话生成自由 HTML 片段，所见即所得预览。布局为**两栏**：左=预览区，右=AI 对话面板；「编辑器」（Monaco）为顶栏开关项，打开后在左栏与预览并排。
