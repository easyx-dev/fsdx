...
- **管理端列表页统一规范：查询状态 / 骨架 / 通用态收敛为固定流水线（[infra]）**：二十余个各写一套的列表页收敛为「骨架负责布局与高度、查询 hook 负责状态与请求、表格负责渲染、单元格负责通用态」，页面只声明差异（列、筛选控件、行操作）。
  ...
  - **编辑承载统一为抽屉**：新增 `AdminFormDrawer`（宽度三档 `640` / `760` / `60%`、固定 `destroyOnHidden`、底部按钮经 HTML `form` 属性提交，表单侧以可选的 `formId` / `hideActions` / `onSubmittingChange` 配合）；删除 `news` 的 `create.tsx` 与 `$id/edit.tsx` 路由页，新建编辑回归列表页抽屉。
...
- **管理端 UI 规范统一为页头三段式，并列宽预算写入硬规则（[infra]）**：把「标题栏 + 独立筛选行」的两段式收敛为**标题栏一行三段**（标题段 | 筛选段 | 操作段），删除整条页面级筛选行，并把「表格不出现横向滚动」变成可实测的硬规则。
  - **规范单一事实来源**：新增 [.agents/skills/admin-design](.agents/skills/admin-design/SKILL.md)（规则本体：页头三段式 / 筛选落点决策表 / 列宽预算 / 操作列 / 弹窗与抽屉 / 表单 / 分析页 / 反例）、[docs/admin-design.md](docs/admin-design.md)（机制与实测依据）、[.agents/checklists/admin-design.md](.agents/checklists/admin-design.md)（验收项）；删除 `docs/admin-list-page.md` 与旧清单，`AGENTS.md` 的「表格操作列」「表格列规范」压缩为规则 + 锚点（数值不复制），`admin-crud` skill 的列表页章节改为引用。
  - **页头三段式**：`AdminPageContent` 输出三级布局（标题块限宽 300px 省略 / `titleTrailing` 筛选段左起 `flex-1` / `extra` 操作段 `ml-auto`）；新增 `AdminFilters`——内联筛选 +「筛选 ▾」浮层（`AdminFilterItem` 带标签、`moreCount` 角标）+ `查询` / `重置` 同组相邻，取代两端对齐、中间大片死区的 `AdminTableToolbar`；`AdminListPage` 的 `toolbar` prop 改为 `filters`。
  - **列宽预算**：`Σ列宽 ≤ 1199`（1440 视口）且页面不得出现横向滚动；唯一主内容列吸收剩余宽度、其余列显式定宽、长文本列必须 `ellipsis`；超出预算按「详情性长文本 → 行展开、次要时间列 → 删、操作列项数 → 更多」裁剪。实测两个易漏点：无 `ellipsis` 的长文本列、以及含四字文案（`重置密码` / `设为默认`）的操作列按 240 估宽，各会产生 10px 级横向滚动，后者需 260。
  - **状态类筛选进列头**：`useListQuery` 新增 `mapColumnFilters`（受控列筛选逐值比较，只在真正变化时回第 1 页，避免纯翻页被重置），列 `filters` / `filteredValue` 直连服务端查询；删除页面级状态快筛 `PublishedFilter` 与三处「整行快筛」（新闻发布状态、文件临时/永久、消息未读/已读）。
  - **操作列与时间格式化**：`TableOperate` 新增 `More` 子组件（低频操作折入下拉，单项可带 `disabledReason`）；`ProTable` 新增 `formatDateTimeValue`，供行展开面板等非表格场景复用与列 `valueType` 同一套时间格式化。
  - **容器统一**：新增 `AdminFormModal`（与 `AdminFormDrawer` 对称：宽度档位 `420` / `520` / `640`、`formId` 触发内部表单提交、标题可按实体名生成或 `title` 覆盖、固定 `destroyOnHidden`、表单不再手写 `mt-4`）；`FORM_DRAWER_WIDTH.full` 由 `"40%"` 修正为 `"60%"`（原值在 1440 视口下 576px 反而窄于 `base 640`）。13 处手写 `Modal` 的新建 / 编辑表单统一迁移（角色、用户及其重置密码、AI 厂商、消息发送与通知设置、元事件、元属性、翻译、配置、字典与字典条目），其中 `footer={null}` + 表单自带按钮的两派写法改为 `formId` 触发容器底部按钮；**弹窗主按钮文案随之由「确定」变为「保存」**（e2e 断言同步）；图片预览、图片编辑器、重命名等工具类弹窗保留原生 `Modal`（已在规范中划定边界）。
  - **迁移范围**：20 个列表页 + 4 个分析页——状态筛选改列头、低频筛选进「筛选 ▾」、筛选行整条删除、列裁剪到预算内（文件管理 12→7 列并把 MIME/路径/SHA256/过期与更新时间收进行展开、字典条目 10→5 列同理、系统配置去掉「描述」列并把分组栏收窄去 Card、UI/实体翻译去掉更新时间并以翻译值为主列、AI 厂商补 `scroll.x`），4 个分析页的筛选从 `Card` 移入页头（粒度 / 对比 / 维度拆解进「筛选 ▾」）。可被衍生项目吸收
  - **双栏（master-detail）抽为共用骨架 `AdminSplitPanel`**：系统配置（分组概览）与字典管理（字典类型）此前各写一套左栏——自绘 `button` 列表 vs `Card` + `div` 点击 + `stopPropagation`——现统一为「左栏定宽档位（180 / 200）+ 列表项（选中指示条 + 主文案 + 次要文案 + 右侧计数 / 行内操作）+ 右栏内容」。左栏与右表采用同一套视觉逻辑（`border-border` 外框 + 灰底标题行，取表格表头的底色与字号，实测两栏等高、表头差 2px）；**左栏固定高度并吸顶**（`calc(100vh - var(--admin-header-height) - 2.5rem)`，实测 804px）、列表区内部滚动，右栏长表滚动时左栏不跟着滚走；列表项整行可点、`border-b border-divider` 分隔、选中用 `border-l-[3px]` + `border-l-transparent` 占位（与侧边导航同构、切换不位移），**交互态补齐**悬停 `bg-accent/60` / 按下 / `focus-visible` 内描边 / `cursor-pointer`（原实现只有不透明的 `bg-accent`，白底上几乎看不见）；行内操作由主色 `link` 改为静默 `text`（悬停提亮为 primary / danger），避免每行两枚主色图标。字典管理右栏去掉 `Card` 外壳与内嵌标题行（当前字典名与条目数改由页头 `description` 承载），「新建条目」上移到页头操作段并在未选中字典时置灰。可被衍生项目吸收
  - **规范一致性复检后的修正**：消息管理的「状态」筛选由页头 Select 收进列头漏斗（与收件箱一致，`mapColumnFilters` 接入）；日志 / 操作日志分析页的「分组维度 / 粒度」与文件管理的「标签搜索」收进「筛选 ▾」（页头只留唯一主搜索）；时间列一律 165（`dateTimeMinute`，原 160/180 混用）；操作列宽度按实测内容宽修正为 2 项短文案 170、3 项含四字文案 270（原 160/260 各差 8px，按钮会溢出到相邻列），编辑器类型列 100→150；配置分组 / 消息用户 / 操作人 / 字典值补 `ellipsis`；非表格时间统一走 `formatDateTimeValue`；权限选择器（弹窗内迷你表）显式给 `scroll.y` 避免继承页面表体高度；工具类弹窗补 `width`（520）与 `destroyOnHidden`，翻译抽屉宽度由 `size={680}` 归入档位 `wide 760`。可被衍生项目吸收
  - **页面说明移入标题 Tooltip**：`AdminPageContent` 的 `description` 不再渲染为标题栏第二行，改为悬停标题时的 Tooltip（`mouseEnterDelay: 0.3`，统一由容器处理）；标题栏 56px 内只剩标题与控件，标题垂直居中，筛选段不再被说明文字挤压，动态说明（如「当前 27 项」）仍保留在该 Tooltip 中。可被衍生项目吸收
- **表格列宽机制化：`scroll.x` 自动推导 + 弹性列 + 双通道守门（[infra]）**：把「余宽给谁、`scroll.x` 写多少」从各页手写收敛为一条可机械执行的规则，并让写错变难。
  - **实测确立宽度模型**：antd 渲染的 `<table>` 内联 `table-layout: fixed; width: {scroll.x}; min-width: 100%`，据此在真实页面注入实测——容器宽于 `scroll.x` 时余宽**全部归不写 `width` 的列**（弹性列），出现横向滚动时该列**恰好等于**声明宽；全列都定宽时浏览器按列宽**等比**分配余宽（7 列 `[80,180,180,160,100,165,270]` 在 1440 下渲染为 `[85,190,190,169,106,174,285]`、在 1120 下严格等于声明值）。于是 `scroll.x` = 各列宽度之和，由代码推导、页面不再手写（此前手写值会与列宽悄悄漂移，曾出现 `scroll.x = 710 < Σ列宽 725` 这类自相矛盾）。
  - **`ProTable` 接管机制**：`scroll.x` 缺省等于各列宽度之和（含行展开 50 / 行选择 48）；`tableLayout` 固定 `fixed`；列上标 `elastic: true` 即渲染时移除 `width`（该 `width` 的语义变为「出现横向滚动时的宽度」）；开发期对「Σ 超预算 / 弹性列 ≠ 1 / 非弹性列缺 `width`」输出 `console.warn`（`AdminProvider` 在 DEV 下开启，生产零输出）。**恰好一列弹性**，优先级：操作列 → 长文本主列 → 首个可伸缩文本列 → 不设。
  - **档位成为单一事实来源**：新增 `COLUMN_WIDTH`（展开 50 / 选择 48 / 头像 80 / 状态 100 / 开关 100 / 排序 115 / 时间 165·180 / 标签 150 / 标识 120·170 / 短文本 180 / 长文本 340）与 `actionsWidth(...labels)`（操作列按「图标 16 + 间距 4 + 文案 + 内边距 16，项间 8，单元格 32」实算并向上取 10：1 项 100 / 2 项 170 / 3 项 240 / 含四字文案 270 / 4 项 320），页面不再写裸数字。
  - **预算按布局下发**：`AdminPageContent` 下发全宽页可用宽，`AdminSplitPanel` 覆盖为右栏实宽，迷你表与演示页可 `budget={null}` 显式关闭校验。**参考视口取 1600（全宽 1359 / 双栏 1157·1137）**：长文本列有 340 的可读下限，按 1440 计会迫使列宽被压到不可读或为凑预算砍列（实测系统配置需 1150、文件管理需 1275）；按 1600 计这几张表在 1600 及以上均不滚动（1920 / 1600 实测横向溢出 0），代价是 1440 视口下最宽的 5 张表需横向滚动 11~151px——有意的取舍：大屏信息完整度优先。
  - **双通道守门**：开发期告警 + `app/src/routes/admin/_admin/__tests__/column-budget.test.ts` 对 11 个列工厂做 CI 校验（每列定宽、弹性列唯一、合计不超预算，新增页面登记即纳入）；`packages/ui-spa` 补 vitest 配置与 `applyColumnBudget` / `COLUMN_WIDTH` 单测。
  - **页面迁移**：默认把余宽给操作列（管理员 / 客户端用户 / 两个角色 / AI 厂商 / 元事件 / 元属性 / 文件管理 / 资源管理器 / 系统配置 / 字典条目），只有确需展示长文本主列的表让位给它（新闻标题 / 消息标题 / 翻译值 ×2 / 运行日志消息 / 操作日志目标 / 埋点属性）——让位时该列按预算给足、操作列退回实测宽（如新闻标题 240 并吸收至 289、翻译值 240 并吸收至 434）；资源管理器由 `minWidth` + `tableLayout="auto"` 改为定宽 + `ellipsis`（auto 布局按 max-content 计算，多列表会被撑爆）；19 处手写 `scroll.x` 全部删除。顺带按新口径修正列宽：AI 厂商「默认」80→100、配置「值类型」150（`Boolean 布尔开关` 标签实测 113px，90 会溢出）、配置键 300→220 与字典标签 300→240，而内容不可控的列（文件名 / 名称 / 配置值 / 字典值 / 标题 / 属性）统一按长文本档位 **340**（低于此值频繁省略、只能靠 Tooltip 读全文；配置值与字典值实测 97 / 172 → 340）。三档实测（1440 / 1280 / 1120）横向溢出与单元格内容溢出均为 0（窄屏下操作列恰好等于声明宽，固定态按钮不被挤出）。可被衍生项目吸收
...
- ⚠️ **管理端页面骨架与筛选 API 变更（[infra]）**：`AdminTableToolbar` 与 `PublishedFilter` 移除（状态类筛选改用列头漏斗，页头筛选改用 `AdminListPage` 的 `filters` + `AdminFilters`，`toolbar` prop 不再存在）；`FORM_DRAWER_WIDTH.full` 由 `"40%"` 改为 `"60%"`（含富文本 / 图片墙的抽屉变宽）；`docs/admin-list-page.md` 与 `.agents/checklists/admin-list-page.md` 删除，改由 `admin-design` 三件套（skill / docs / checklist）承载。
