...
- **管理端 UI 规范统一为页头三段式，并列宽预算写入硬规则（[infra]）**：把「标题栏 + 独立筛选行」的两段式收敛为**标题栏一行三段**（标题段 | 筛选段 | 操作段），删除整条页面级筛选行，并把「表格不出现横向滚动」变成可实测的硬规则。
  ...
  - **列宽表达与 `scroll.x` 口径写进规范**：`width`（内容长度可预期，精确渲染）/ `minWidth` + `tableLayout="auto"`（内容都短且差异大，按内容分摊、不低于最小宽）/ 不给宽度（吸收剩余）三者选一，并**要求至少一列不定宽、`scroll.x` 必填**（= 参考视口 1440 下该表可用宽度：全宽页 1199、双栏页 997 / 977）——容器大于该值时不定宽列吸收多余宽度，等于时恰好铺满，小于时横向滚动且列宽不被压缩。资源管理器因此改为 4 列 `minWidth` + `auto` 布局（去掉操作列的定宽与 `fixed`），埋点事件查询把「属性」改为不定宽；实测资源管理器在 1280 压到最小宽 320/150/180/320、在 1200 才出现 11px 横向滚动。**边界**：auto 布局按内容 max-content 计算，长文本表会被撑爆（演示页 8 列撑到 2364px），故长文本表必须用 `width` + `ellipsis` 的 fixed 布局。可被衍生项目吸收
  - 行展开列由 90 收窄到 **50**（`+` 图标 16 + 单元格内边距 32 = 48，antd 默认即 48），腾出的 40px 给不定宽列（文件管理文件名 184→224）。可被衍生项目吸收
  - 同批统一列宽口径：字典「标签」与配置「配置键」两个**标识类主列定宽 300**（内容长度可控），而内容不可控的「配置值」保持**吸收剩余**（实测 149，定宽会让密钥 / JSON 频繁省略）；**行展开列统一 90**（`expandable.columnWidth`，antd 默认 48 会让 `+` 与行缩进挤在一起），文件管理为此把「标签」200→170、「状态」130→120，字典条目把「值」180→140；**排序列统一 115、状态列统一 100**（`SortOrderCell` 内含 78px 输入框；`PublishSwitchCell` 相应改用 `size="small"`，标准尺寸开关 78px 在 100px 列内会溢出）、配置「值类型」按实测内容宽 113px 提到 150（原 100px 会让 `Boolean 布尔开关` 标签横向溢出 45px 到相邻列）、字典「值」改回弹性吸收剩余；`scroll.x` 统一取所有定宽列之和（含展开列）。实测 config / users / files / dicts / track-query / logs / operation-logs 的横向溢出与**单元格内容溢出均为 0**。可被衍生项目吸收
